...
function Home() {
  ...
  const [movieNewData, setMovieNewData] = useState([]);
    ...
    fetch(
      ...
      .then((data) => {
        ...
        for (let i = 0; i < data.movies.length; i++) {
          ...
          newTab.push(dataMovie);
          console.log(newTab);
        }
        setMovieNewData(newTab);
      });
  }, []);

  const movies = movieNewData.map((data, i) => {
    const isLiked = movieNewData.some((movie) => movie === data.title);
    return (
      <Movie
        key={i}
